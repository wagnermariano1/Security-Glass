...
const DB = {
    ...
    saveVehicles: async (vehicles) => {
        ...
        // Sincronizar com Firebase usando BATCH (evita rate limit!)
...
const saveBoth = {
    vehicles: async (vehicles) => {
        // DB.saveVehicles já chama batch, não precisa chamar de novo!
        ...
        if (window.firebase && FirebaseDB.initialized) {
            try {
                ...
                if (vehiclesFirebase && vehiclesFirebase.length > 0) {
                    ...
                    // Cache já atualizado pelo listener
                }
            } catch (error) {
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (ultimaVersao && ultimaVersao !== VERSAO_ATUAL) {
        ...
        if (versaoAntigaNumero > 0 && versaoAntigaNumero < VERSAO_MINIMA_PERMITIDA) {
            console.error(`❌ Versão em cache ${ultimaVersao} está desatualizada! Mínima: v${VERSAO_MINIMA_PERMITIDA}`);
                ...
                aviso.innerHTML = `
                    <div style="background: #fee2e2; border: 3px solid #dc2626; padding: 24px; border-radius: 12px; text-align: center;">
                        ...
                        <p style="margin: 0 0 12px 0; font-size: 1.1rem; line-height: 1.6;">
                            Sua versão (<strong>${ultimaVersao}</strong>) está desatualizada e <strong style="color: #dc2626;">pode causar perda de dados</strong>!
                        </p>
